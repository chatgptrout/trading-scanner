...
st.set_page_config(page_title="AI TRADEX PRO V167", layout="wide")
...
# --- 2. AI BRAIN: PCR & MOMENTUM ---
pcr_val = 2.01 # Latest from screenshot
ai_signal = "REVERSAL RISK ⚠️" 
ai_conf = "85%"
...
st.markdown(f"""
    <div style='text-align:center; padding:15px; background:#000; color:white; border-radius:10px; border-bottom:5px solid #00c853;'>
        ...
        <div style='background:orange; padding:5px 20px; border-radius:5px; display:inline-block; font-weight:bold;'>
            SIGNAL: {ai_signal} | CONFIDENCE: {ai_conf}
        </div>
    </div>
""", unsafe_allow_html=True)

# --- 3. LIVE CARDS WITH DYNAMIC LEVELS ---
# Symbols as per latest live feed
...
for i, (name, sym) in enumerate(symbols.items()):
    ...
    if not df.empty:
        ...
        with cols[i]:
            st.markdown(f"""
                <div style='border:1px solid #333; padding:15px; border-radius:10px; text-align:center; background:#111; color:white;'>
                    <h5 style='color:gray; margin:0;'>{name}</h5>
                    <h2 style='margin:5px 0;'>{price_str}</h2>
                    <hr style='border:0.1px solid #333;'>
                    <div style='color:#00c853; font-size:11px; font-weight:bold;'>BULLISH ABOVE: {hi:.2f}</div>
                    <div style='color:#ff1744; font-size:11px; font-weight:bold;'>BEARISH BELOW: {lo:.2f}</div>
                </div>
            """, unsafe_allow_html=True)

# --- 4. AI SCANNER TABLE ---
# SUNPHARMA (1725.0) & NTPC (373.0)
stocks_data = [
