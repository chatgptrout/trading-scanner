...
with st.sidebar:
    ...
    <div style='text-align:center; padding:15px; background:#fff; border:1px solid #eee; border-radius:15px;'>
        <div class='pcr-container' style='background:conic-gradient({p_color} {int(pcr_val*50)}%, #eee {int(pcr_val*50)}%);'>
            <div class='pcr-inner'>
                <small style='color:gray; font-weight:bold;'>PCR</small>
                <b style='font-size:24px; color:{p_color};'>{pcr_val}</b>
            </div>
        </div>
...
for s in ["RELIANCE.NS", "TCS.NS", "HDFCBANK.NS", "ICICIBANK.NS", "SBIN.NS", "BHARTIARTL.NS"]:
    ...
    if val:
        ...
        st.markdown(f"""
        <div class='table-row'>
            <div style='width:25%; font-weight:bold;'>{t}</div>
            <div style='width:20%'><span style='background:{bg}; color:{clr}; padding:5px 10px; border-radius:6px; font-weight:bold; font-size:11px;'>{sig}</span></div>
            <div style='width:35%; color:gray; font-size:12px;'><b>{l_lbl}</b>: {val['ema']}</div>
