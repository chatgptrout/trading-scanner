...
def get_market_status(symbol):
    ...
    if symbol in ["^NSEI", "^BSESN"]: # Equity timings
        start, end = now.replace(hour=9, minute=15), now.replace(hour=15,30)
        return "LIVE 🟢" if start <= now <= end and now.weekday() < 5 else "CLOSED 🔴"
    else: # Commodity timings (MCX)
        start, end = now.replace(hour=9, minute=0), now.replace(hour=23,55)
        return "LIVE 🟢" if start <= now <= end and now.weekday() < 5 else "CLOSED 🔴"

# --- 2. PCR HEADER (FROZEN AFTER 3:30) ---
pcr_val = 2.01 # Latest
st.markdown(f"""
    <div style='text-align:center; padding:10px; border-bottom:3px solid #00c853;'>
        ...
        <p style='color:orange; font-weight:bold;'>AI FORECAST: OVERBOUGHT (CAUTION)</p>
    </div>
...
# --- 4. SCANNER (STOPS AFTER 3:30) ---
if "LIVE" in get_market_status("^NSEI"):
    st.markdown("<br>### 🚀 AI POWER SCANNER (RUNNING)")
    # (Stock table logic here)
else:
    st.info("Equity Scanner is sleeping. Commodity is active! 📦")
